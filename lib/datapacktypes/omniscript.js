    ...
    if (dataPack.VlocityDataPackData['%vlocity_namespace%__OmniScript__c'][0]['%vlocity_namespace%__IsLwcEnabled__c']) {
        try {
            ...
            if (!puppeteerOptions.executablePath && !jobInfo.puppeteerInstalled) {
                puppeteerOptions.executablePath = chromium.path;
            } 
            ...
            if(!jobInfo.checkLWCActivationPage){
                try {
                    VlocityUtils.log('LWC Activation', 'Checking for Supported Manage Package Version');
                    var notPageMessage = await page.$x("//*[contains(., 'URL No Longer Exists')]", {timeout: 5000});
                    if(notPageMessage && notPageMessage.length > 0){
                        jobInfo.ignoreLWCActivation = true;
                        VlocityUtils.error('LWC Activation', 'OmniScript LWC will not be activated. Unsupported Manage Package Version');
                        return;
                    }
                    jobInfo.checkLWCActivationPage = true; 
                    VlocityUtils.log('LWC Activation', 'OmniScript LWC will be Activated. Supported Manage Package Version');
                } catch (error) {
                    jobInfo.checkLWCActivationPage = true; 
                    VlocityUtils.log('LWC Activation', 'OmniScript LWC will be Activated. Supported Manage Package Version');
                }
            ...
            //VlocityUtils.log('LWC Activation Link', omniScriptDisignerpageLink);
            let tries = 0;
            var errorMessage;
            while (tries < 10 && !jobInfo.ignoreLWCActivation) {
                try {
            ...
            if(e.message.includes('Failed to launch the browser process')){
                jobInfo.ignoreLWCActivation = true;
                VlocityUtils.log('Activating LWC', 'OmniScript LWC Activation disabled',);
            }
        }
    }
}
